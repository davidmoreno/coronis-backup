...
TAR_ERRORS = {
    1: "Modified while creating backup. "
...
def ssh(host, script, simulate=None, **kwargs):
    ...
    try:
        ...
        logging.error(
            ...
            % (host["host"], e.exit_code, host, script, e)
        )
        return False
...
def backup(host, path, gpg_key=None):
    ...
    if tar:
        if incremental:
            mtime = (
                datetime.datetime.now() - datetime.timedelta(days=incremental)
            ).strftime("%Y%m%d")
            ...
            return backup_stdout(
                host, outfile, ["tar", "cz", path], gpg_key, error_codes=TAR_ERRORS
            )
    else:
...
def backup_stdout(host, name, cmd, gpg_key=None, error_codes={}):
    global all_ok
    hostname = host["host"]
    outfile = "%s/%s/%s/%s" % (destdir, date, host["host"], name.replace("/", "-"))
    ...
    if gpg_key:
        if not simulate:
            recipients = [
                x for recipient in wrapped(gpg_key) for x in ("-r", recipient)
            ]
            gpgout = sh.gpg2(
                gencmd,
                "-e",
                ...
                if gpgout.exit_code != 0:
                    ...
                    for err, msg in error_codes.items():
                        if gencmd.exit_code == err:
                            logging.warning("[%s] %s" % (hostname, msg))
                            ok = True
                    if not ok:
    ...
    if not simulate:
        try:
            ...
            logging.info(
                "[%s] %s -- %.2f MB" % (hostname, outfile, size / (1024 * 1024.0))
            )
            if size == 0:
                logging.warning("[%s] %s is EMPTY!" % (hostname, outfile))
            elif size < 1024:
                logging.warning(
                    "[%s] %s is TOO small! (%s bytes)" % (hostname, outfile, size)
                )
        except Exception:
            all_ok = False
            ok = False
...
def host_auth(hostname):
    ...
    return {"host": hostname, **data.get("auth", {})}


def read_all_auths():
    print(backup_plan)
    ret = [host_auth(i) for i in backup_plan.keys() if i != "default"]
...
def email_stats():
    ...
    for email, emaild in stats.items():
        ...
        table += (
            "<tr style='background: #2185d0; color:white; '><th>Host</th><th>Area</th>"
        )
        ...
        if "@" in email:
            logging.info(
                "Send email statistics to %s: %s" % (email, "OK" if all_ok else "ERROR")
            )
            smtp = backup_plan.get("smtp", {})
            server = smtplib.SMTP(
                smtp.get("hostname", "localhost"), smtp.get("port", 587)
            )
...
def parse_options():
    ...
    OPTIONS = (
        "ih",
        ["since=", "dry", "simulate", "incremental", "help", "full", "p=", "plan="],
    )
    try:
    ...
    assert os.path.isdir(destdir), "Need a backup directory file as first argument"
...
def main():
    ...
    for h in hosts:
        try:
            ...
            update_stats(
                backup_plan.get("default", {}).get("mailto", []),
                h["host"],
                "*",
                "*",
                False,
